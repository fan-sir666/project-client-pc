    ...
    <div class="xtx-pay-page">
      <div class="container">
        ...
        <div class="pay-result">
          ...
          <p>
            支付金额：<span>¥{{ Number(payMoney).toFixed(2) }}</span>
          </p>
          <div class="btn">
            <XtxButton
              type="primary"
              style="margin-right: 20px"
              @click="$router.push(`/member/order/${id}`)"
              >查看订单</XtxButton
            >
            <XtxButton type="gray" @click="$router.push('/')"
              >进入首页</XtxButton
            >
          </div>
          <p class="alert">
            <span class="iconfont icon-tip"></span>
            温馨提示：冬奥优品不会以订单异常、系统升级为由要求您点击任何网址链接进行退款操作，保护资产、谨慎操作。
          </p>
        </div>
      </div>
    </div>
