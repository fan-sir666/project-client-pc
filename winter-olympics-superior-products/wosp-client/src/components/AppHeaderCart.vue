  ...
  <div class="cart">
    ...
    <div class="layer" v-if="effectiveGoodsCount > 0 && !isCartPage">
      <div class="list">
        <div
          class="item"
          v-for="item in effectiveGoodsList"
          :key="item.cart_id"
        >
          <RouterLink :to="`/goods/${item.goods_id}`">
          ...
          <i
            ...
            @click="deleteGoodsOfCartBycartId(item.cart_id)"
          ></i>
        </div>
      </div>
      <div class="foot">
...
export default {
  ...
  setup() {
    ...
    // 删除商品
    const deleteGoodsOfCartBycartId = (id) => {
      Confirm({
        content: "您确定要删除该商品吗?",
      })
